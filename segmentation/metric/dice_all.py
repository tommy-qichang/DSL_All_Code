...
def one_cls_dice(pred, target, label_idx):
    """
    Calculate Dice metrics for one channel
    :param output:Output dimension: Batch x X x Y (x Z) int
...
def dice_all(output, target, misc=None):
    """
    ...
    for i in range(1, channel_num):
        dice = one_cls_dice(pred, target, label_idx=i)
        dices.append(dice)
