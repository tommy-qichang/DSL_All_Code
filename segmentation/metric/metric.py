...
def __distinct_binary_object_correspondences(reference, result, connectivity=1):
    ...
    for l1id, slicer in enumerate(slicers):  # iterate over object in labelmap2 and their windows
        ...
        l2ids = numpy.unique(
            labelmap1[slicer][bobj])  # extract all unique object identifiers at the corresponding positions in the reference (i.e. the mapping)
...
def distance_metrics(result, reference, voxelspacing=None, connectivity=1):
    '''
    compute asd, assd, hd, hd95, hd98
    :param result: binary mask array of result
    :param reference: binary mask array of reference
    :param voxelspacing: spacing
    :param connectivity: connectivity of connected component
    :return: asd, assd, hd, hd95, hd98
    '''
    dist_result_2_ref = __surface_distances(result, reference, voxelspacing, connectivity)
    dist_ref_2_result = __surface_distances(reference, result, voxelspacing, connectivity)
    dist_all = numpy.concatenate((dist_ref_2_result, dist_result_2_ref))
    # asd = dist_result_2_ref.mean()
    assd = dist_all.mean()
    # hd = dist_all.max()
    # hd95 = numpy.percentile(dist_all, 95)
    hd95 = (numpy.percentile(dist_result_2_ref, 95) + numpy.percentile(dist_ref_2_result, 95)) / 2.0
    return assd, hd95