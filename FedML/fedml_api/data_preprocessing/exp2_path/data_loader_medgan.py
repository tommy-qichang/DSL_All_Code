...
from .data_utility import init_transform, count_samples, build_pairs, build_pairs_complete
...
def get_dataloader(h5_train, h5_test, train_bs, test_bs, channel_in=3):
    ...
    if h5_train is not None:
        ...
        train_dl = data.DataLoader(dataset=train_ds, batch_size=train_bs, shuffle=True, drop_last=True, pin_memory=False)
    else:
        train_dl = None
    ...
    if h5_test is not None:
        ...
        test_dl = data.DataLoader(dataset=test_ds, batch_size=test_bs, shuffle=False, drop_last=False, pin_memory=False)
    else:
        test_dl = None
...
def load_partition_data_distributed_path(process_id, dataset, data_dir, partition_method, partition_alpha,
    ...
    train_data_num = None #count_samples(data_dict['train_all'], 'images')
    class_num = 2  # not accurate, but not used in fedgan
...
class GeneralDataset(data.Dataset):
    def __init__(self, h5_filepath, channel_in=3, transforms=None, sample_rate=1.0, has_label=True):
        """
        ...
        self.im, self.label, self.keys = build_pairs(h5_file, sample_rate)
        h5_file.close()
...
class TestDataset(data.Dataset):
    def __init__(self, h5_filepath, channel_in=3, transforms=None, sample_rate=1.0, has_label=True):
        ...
        h5_file = h5py.File(self.h5_filepath, 'r')
        # train_db = h5_file[path]
        self.im, self.label, self.labels_ternary, self.weight_maps, self.keys = build_pairs_complete(h5_file, sample_rate)
        h5_file.close()
    ...
    def __getitem__(self, index):
        ...
        weight_maps = self.weight_maps[index]

        datapoint = {'image': B, 'mask': A, "misc": {
                "index": index,
                "key": key,
                "labels_ternary": labels_ternary,
        ...
        return {'A': datapoint['mask'], 'B': datapoint['image'], 'key': key, "labels_ternary": datapoint['misc']['labels_ternary'], "weight_maps": datapoint['misc']['weight_maps']}
