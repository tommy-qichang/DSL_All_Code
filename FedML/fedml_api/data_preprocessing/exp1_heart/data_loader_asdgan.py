...
def get_transforms_G():
    ...
    transforms_args_test = {
        ...
        "NormalizeBoth": [0.5, 0.5]  # only normalize image
    }
...
def partition_data(datadir, partition, n_clients):
    ...
    data_dict = dict()
    data_dict['test_all'] = os.path.join(datadir, 'whs_ct_train_2d_iso.h5')  # for visualization debug only
    ...
    # non-iid data distribution
        ...
        data_dict[0] = os.path.join(datadir, 'whs_ct_train_2d_iso.h5')
        data_dict[1] = os.path.join(datadir, 'miccai2008_train_2d_iso.h5')
        data_dict[2] = os.path.join(datadir, 'asoca_train_2d_iso.h5')

    return data_dict


def load_partition_data_distributed_heart(process_id, dataset, data_dir, partition_method,
...
class DatasetG(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # return torch.from_numpy(np.array(client_ids, dtype=np.uint8)), torch.tensor(As, dtype=torch.float32), torch.from_numpy(np.array(keys_id))
...
class DatasetD:
    def __init__(self, h5_filepath, im_size=(256, 256), transforms=None, path="train", sample_rate=1, channel_in=1):
        """
        ...
        self.dcm, self.label, self.keys = build_pairs(train_db, sample_rate, im_size)
        h5_file.close()
...
class TestDataset(data.Dataset):
    def __init__(self, h5_filepath, transforms=None, path="train", sample_rate=0.5, channel_in=1):
        """
        :param h5_filepath:string h5 file path to load.
        :param transforms: my_transforms
        :param paths: array of all keys which will loaded
        ...
        h5_file = h5py.File(self.h5_filepath, 'r')
        train_db = h5_file[path]
        self.dcm, self.label, self.keys = build_pairs(train_db, sample_rate)
        h5_file.close()
