...
class FedSegAggregator(object):
    def __init__(self, worker_num, device, model, args, model_trainer, train_data_global, test_data_global):
        ...
        self.train_dice_client_dict = dict()
        ...
        self.test_dice_client_dict = dict()
        ...
        logging.info('[Server] Initializing FedSegAggregator with workers: {0}'.format(worker_num))
    ...
    def add_client_test_result(self, round_idx, client_idx, train_eval_metrics:EvaluationMetricsKeeper, test_eval_metrics:EvaluationMetricsKeeper):
        ...
        if (round_idx+1) % self.args.evaluation_frequency == 0:
            ...
            self.train_dice_client_dict[client_idx] = train_eval_metrics.dice
            ...
            train_eval_metrics_dict = {
                ...
                'dice': self.train_dice_client_dict[client_idx]
            }
            logging.info("[Server] Training statistics of client {0}: {1}".format(client_idx, train_eval_metrics_dict))
        ...
        self.test_dice_client_dict[client_idx] = test_eval_metrics.dice
        ...
        test_eval_metrics_dict = {
            ...
            'dice': self.test_dice_client_dict[client_idx]
        }
        ...
        if self.args.save_client_model:
            ...
            if test_mIoU > best_mIoU:
                ...
                logging.info('[Server] Saving Model Checkpoint for Client: {0} --> Previous mIoU:{1}; Improved mIoU:{2}'.format(client_idx, best_mIoU, test_mIoU))
                is_best = False
                filename = "client" + str(client_idx) + "_checkpoint.pth.tar"
                saver_state = {'best_pred': test_mIoU, 'round': round_idx + 1, 'state_dict': self.model_dict[client_idx],
                               'test_data_evaluation_metrics': test_eval_metrics_dict}
    ...
    def output_global_acc_and_loss(self, round_idx):
        ...
        stats_test = {'testing_acc': test_acc,
                    ...
                    'testing_dice': test_dice}

        logging.info("[Server] Testing statistics: {}".format(stats_test))

        if test_dice > self.best_dice:
            logging.info('[Server] Saving Best Model Checkpoint --> Previous dice:{0}; Improved dice:{1}'.format(self.best_dice, test_dice))
            is_best = True
            self.best_dice = test_dice
            saver_state = {'best_pred': self.best_dice, 'round': round_idx + 1, 'state_dict': self.trainer.model.state_dict(),
                           'test_data_evaluation_metrics': stats_test}
        ...
        if (round_idx+1) % self.args.evaluation_frequency == 0:
            ...
            saver_state = {'best_pred': test_dice, 'round': round_idx + 1, 'state_dict': self.trainer.model.state_dict(),
                           'test_data_evaluation_metrics': stats_test, 'train_data_evaluation_metrics': stats_train}
            self.saver.save_checkpoint(saver_state, False, filename)

        # train_eval_metrics = None
        # if self.train_data_global and (round_idx + 1) % self.args.evaluation_frequency == 0:
        #     train_eval_metrics = self.trainer.test_train(self.train_data_global, self.device)
        #
        # test_eval_metrics = self.trainer.test(self.test_data_global, self.device)
        #
        # if train_eval_metrics is not None:
        #     # Test on training set
        #     train_acc = train_eval_metrics.acc
        #     train_acc_class = train_eval_metrics.acc_class
        #     train_mIoU = train_eval_metrics.mIoU
        #     train_FWIoU = train_eval_metrics.FWIoU
        ...
        #     stats_train = {'acc': train_acc,
        #                    'acc_class': train_acc_class,
        #                    'mIoU': train_mIoU,
        #                    'FWIoU': train_FWIoU,
        #                    'loss': train_loss,
        #                    'dice': train_dice}
        ...
        #
        # # Test on testing set
        # test_acc = test_eval_metrics.acc
        # test_acc_class = test_eval_metrics.acc_class
        # test_mIoU = test_eval_metrics.mIoU
        # test_FWIoU = test_eval_metrics.FWIoU
        # test_loss = test_eval_metrics.loss
        # test_dice = test_eval_metrics.dice
        #
        # # Test Logs
        ...
        # stats_test = {'acc': test_acc,
        #               'acc_class': test_acc_class,
        #               'mIoU': test_mIoU,
        #               'FWIoU': test_FWIoU,
        #               'loss': test_loss,
        #               'dice': test_dice}
        ...
        #
        # if test_dice > self.best_dice:
        #     logging.info('[Server] Saving Model Checkpoint --> Previous dice:{0}; Improved dice:{1}'.format(self.best_dice, test_dice))
        #     is_best = True
        #     self.best_dice = test_dice
        #     saver_state = {'best_pred': self.best_dice, 'round': round_idx + 1, 'state_dict': self.trainer.model.state_dict(),
        #                    'test_data_evaluation_metrics': stats_test}
