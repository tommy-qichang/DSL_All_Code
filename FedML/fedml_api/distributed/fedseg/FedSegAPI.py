...
def FedML_FedSeg_distributed(process_id, worker_number, device, comm, model, train_data_global, test_data_global,
...
def init_server(args, device, comm, rank, size, model, model_trainer, train_data_global, test_data_global):
    ...
    # aggregator
    worker_num = size - 1
    aggregator = FedSegAggregator(worker_num, device, model, args, model_trainer, train_data_global, test_data_global)
...
def init_client(args, device, comm, process_id, size, model, train_data_local_num_dict,
    ...
    # trainer
    trainer = FedSegTrainer(client_index, train_data_local_dict, train_data_local_num_dict, test_data_local_dict, device,
                            args, model_trainer)
