...
def convert_to_img(A, B, fake_B):
    ...
    if len(label.shape) == 3:
        label = label[0]
    label = float_to_uint_img(label, None, 0, minv=-1, maxv=1)
...
class MyModelTrainer(ModelTrainer):
    ...
    def get_model_params(self, model_name=None):
        logging.info('[{}] Obtain model parameters'.format(self.node_name))
        return self.model.get_weights(model_name)

    def set_model_params(self, model_parameters, model_name=None):
        logging.info('[{}] Updating model'.format(self.node_name))
        self.model.load_weights(model_parameters, model_name)
