...
class FedMedGanAggregator(object):
    def __init__(self, worker_num, device, model, args, model_trainer, train_data_global, test_data_global):
        ...
        logging.info('[Server] Initializing FedMedGanAggregator with workers: {0}'.format(worker_num))

    def get_global_model_params(self):
        return self.trainer.get_model_params(['G'])

    def set_global_model_params(self, model_parameters):
        self.trainer.set_model_params(model_parameters, ['G'])
    ...
    def aggregate(self):
        ...
        for net in ['netG']:  # averaged_params.keys():
    ...
    def output_global_acc_and_loss(self, round_idx):
        ...
        if (round_idx+1) % self.args.evaluation_frequency == 0:
            ...
            saver_state = {'round': round_idx + 1, 'state_dict': self.trainer.model.get_weights(['G']),
            ...
            if self.args.save_client_model:
                for client_idx in range(self.worker_num):
                    ...
                    filename = "client_{0}_checkpoint_ep{1}.pth.tar".format(client_idx, round_idx + 1)
