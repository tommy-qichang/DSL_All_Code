...
from fedml_api.model.cv.dadgan_mc import DadganMCModel
from fedml_api.distributed.fedmedgan.FedMedGanAPI import FedML_init, FedML_FedMedGan_distributed
from fedml_api.distributed.fedmedgan.utils import count_parameters
...
def add_args(parser):
    ...
    parser.add_argument('--input_nc', type=int, default=None, help='# of input image channels of G: 3 for RGB and 1 for grayscale')
    parser.add_argument('--output_nc', type=int, default=None, help='# of output image channels of G: 3 for RGB and 1 for grayscale')
    ...
    parser.add_argument('--continue_train', default=None, type=str2bool, help='continue training: load the latest model')
...
def load_data(process_id, args):
    if args.dataset[:5].lower() == "brats":
        from fedml_api.data_preprocessing.brats.data_loader_medgan import load_partition_data_distributed_brats as data_loader
    elif args.dataset.lower() == 'path':
        from fedml_api.data_preprocessing.exp2_path.data_loader_medgan import load_partition_data_distributed_path as data_loader
    elif args.dataset.lower() == 'heart':
        from fedml_api.data_preprocessing.exp1_heart.data_loader_medgan import load_partition_data_distributed_heart as data_loader
    ...
    train_data_num, train_data_global, test_data_global, local_data_num, train_data_local, test_data_local, class_num = data_loader(
        ...
        args.client_num_in_total, args.batch_size, args.input_nc)
...
def create_model(args, device):
    if args.model.lower() == 'dadgan':
        model = DadganModel(args, device)

        num_params = count_parameters(model)
        logging.info("dadgan Model Size = {0} M".format(str(num_params)))
    elif args.model.lower() == 'dadgan_mc':
        model = DadganMCModel(args, device)

        num_params = count_parameters(model)
        logging.info("dadgan_MC Model Size = {0} M".format(str(num_params)))
    else:
...
if __name__ == "__main__":
    ...
    logging.info("Executing FedMedGAN at time: {0}".format(time_start))
    ...
    str_process_name = "FedMedGAN (distributed):" + str(process_id)
    ...
    if process_id == 0:
        wandb.init(
            project="fedml",
            name="FedMedGan(d)" + args.dataset + "-r" + str(args.comm_round) + "-e" + str(args.epochs),
            config=args
        )
    ...
    logging.info("Calling FedML_FedMedGan_distributed")

    # start "federated Med GAN (FedMedGan)"
    FedML_FedMedGan_distributed(process_id, worker_number, device, comm, model, train_data_global, test_data_global, data_local_num_dict,
