...
def add_args(parser):
    ...
    parser.add_argument('--no_dropout', default=None, action=str2bool, help='no dropout for the generator')
    ...
    parser.add_argument('--continue_train', default=None, action=str2bool, help='continue training: load the latest model')
    ...
    parser.add_argument('--verbose', default=None, action=str2bool, help='if specified, print more debugging information')
...
def load_data(process_id, args):
    if args.dataset[:5].lower() == "brats":
        from fedml_api.data_preprocessing.brats.data_loader_gan import load_partition_data_distributed_brats as data_loader
    elif args.dataset.lower() == 'path':
        from fedml_api.data_preprocessing.exp2_path.data_loader_gan import load_partition_data_distributed_path as data_loader
    elif args.dataset.lower() == 'heart':
        from fedml_api.data_preprocessing.exp1_heart.data_loader_gan import load_partition_data_distributed_heart as data_loader
...
if __name__ == "__main__":
    ...
    if process_id == 0:
        wandb.init(
            project="fedml",
            name="FedGan(d)" + args.dataset + "-r" + str(args.comm_round) + "-e" + str(args.epochs),
            config=args
        )
    ...
    FedML_FedGan_distributed(process_id, worker_number, device, comm, model, train_data_global, test_data_global, data_local_num_dict,
