...
def save_data(gt, visuals, index, model, file, label_ternary, weight_map, append_idx=0):
    ...
    syn_img = np.moveaxis(syn_img.cpu().numpy().astype("uint8"), 0, -1)
    label = np.moveaxis(label.cpu().numpy(), 0, -1)

    real_img = (real_img + 1) * (255/2)
    real_img = np.rint(real_img.cpu().numpy()).clip(0, 255).astype(np.uint8)
    real_img = np.moveaxis(real_img, 0, -1)

    file.create_dataset(f"images/{key}", data=syn_img)
    file.create_dataset(f"labels/{key}", data=label)
    file.create_dataset(f"reference_real_image_please_dont_use/{key}", data=real_img)
    file.create_dataset(f"labels_ternary/{key}", data=label_ternary)
    file.create_dataset(f"weight_maps/{key}", data=weight_map.cpu().numpy())
...
if __name__ == '__main__':
    ...
    folder_name = f"Nuclei_Dadgan_{opt.netG}_epoch{opt.epoch}"
