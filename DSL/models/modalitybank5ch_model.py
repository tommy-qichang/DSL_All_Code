...
# Include 4db MRI + 1db CT
class Modalitybank5chModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['real_A_2', 'fake_B_2_ch1', 'fake_B_2_ch2', 'fake_B_2_ch3', 'fake_B_2_ch4', 'real_B_2_ch1', 'real_B_2_ch2',
                             'real_B_2_ch3', 'real_B_2_ch4',
                             'real_A_7', 'fake_B_7_ch1', 'fake_B_7_ch2', 'fake_B_7_ch3', 'fake_B_7_ch4', 'real_B_7_ch1', 'real_B_7_ch2',
                             'real_B_7_ch3', 'real_B_7_ch4'
                             'real_A_11', 'fake_B_11_ch1', 'fake_B_11_ch2', 'fake_B_11_ch3', 'fake_B_11_ch4', 'real_B_11_ch1',
                             'real_B_11_ch2', 'real_B_11_ch3', 'real_B_11_ch4'
                             ,'real_A_15', 'fake_B_15', 'real_B_15'
                             ]
        # specify the models you want to save to the disk. The training/test scripts will call <BaseModel.save_networks> and <BaseModel.load_networks>
        self.D_size = opt.d_size  # 4
        ...
        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            self.netD = []
            # for i in range(10):
            for i in range((self.D_size-1) * 4):
            ...
            #the last D for stroke image which just have one channel.
            self.netD.append(networks.define_D(opt.input_nc + opt.output_nc, opt.ndf, opt.netD,
                                               opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain,
                                               self.gpu_ids, opt.memory_gan, current_memory_gan_id=4))
    ...
    def set_input(self, input):
        ...
        self.real_B_2_ch4 = grayimg(self.real_B[0][:, 3])
        ...
        self.real_B_7_ch4 = grayimg(self.real_B[1][:, 3])
        ...
        self.real_B_11_ch4 = grayimg(self.real_B[2][:, 3])
        self.real_A_15 = self.real_A[3]
        self.real_B_15 = self.real_B[3]
    ...
    def forward(self):
        ...
        self.fake_B = []
        for i in range(len(self.real_A)-1):
            self.fake_B.append(self.netG(self.real_A[i]))# G(A)
        self.fake_B.append(self.netG(self.real_A[3],4))
        ...
        self.fake_B_2_ch4 = grayimg(self.fake_B[0][:, 3])
        ...
        self.fake_B_7_ch4 = grayimg(self.fake_B[1][:, 3])
        ...
        self.fake_B_11_ch4 = grayimg(self.fake_B[2][:, 3])
        self.fake_B_15 = self.fake_B[3]
    ...
    def backward_D(self):
        ...
        #CT the last channel
        real_A_1 = self.real_A[3]
        fake_B_1 = self.fake_B[3]
        real_B_1 = self.real_B[3]
        fake_AB = torch.cat((real_A_1, fake_B_1), 1)
        real_AB = torch.cat((real_A_1, real_B_1), 1)

        pred_fake = self.netD[9](fake_AB.detach())
        self.loss_D_fake.append(self.criterionGAN(pred_fake, False))
        pred_real = self.netD[9](real_AB)
        self.loss_D_real.append(self.criterionGAN(pred_real, True))
    ...
    def backward_G(self):
        ...
        pred_fake = self.netD[9](fake_AB)
        self.loss_G_GAN.append(self.criterionGAN(pred_fake, True))
        self.loss_G_L1.append(self.criterionL1(fake_B_1, real_B_1) * self.opt.lambda_L1)
        pred_feat = self.vgg_model(fake_B_1)
        target_feat = self.vgg_model(real_B_1)
        self.loss_G_perceptual.append(
            self.criterion_perceptual(pred_feat, target_feat) * self.opt.delta_perceptual)
