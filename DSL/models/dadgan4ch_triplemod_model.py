...
class Dadgan4chTriplemodModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['real_A_0', 'fake_B_0_ch1', 'fake_B_0_ch2', 'fake_B_0_ch3', 'fake_B_0_ch4', 'real_B_0_ch1', 'real_B_0_ch2',
                             'real_B_0_ch3', 'real_B_0_ch4',
                             'real_A_1', 'fake_B_1_ch1', 'fake_B_1_ch2', 'fake_B_1_ch3', 'fake_B_1_ch4', 'real_B_1_ch1', 'real_B_1_ch2',
                             'real_B_1_ch3', 'real_B_1_ch4',
                             'real_A_2', 'fake_B_2_ch1', 'fake_B_2_ch2', 'fake_B_2_ch3', 'fake_B_2_ch4', 'real_B_2_ch1',
                             'real_B_2_ch2', 'real_B_2_ch3', 'real_B_2_ch4']
        ...
        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            ...
            for i in range(self.D_size * 3):
                self.netD.append(networks.define_D(opt.input_nc + opt.output_nc -2, opt.ndf, opt.netD,
    ...
    def set_input(self, input):
        ...
        for i in range(self.D_size):
            ...
            self.mask.append(input['Mask_' + str(i)].to(self.device))
        ...
        self.real_A_0 = self.real_A[0]
        self.real_B_0_ch1 = grayimg(self.real_B[0][:, 0])
        self.real_B_0_ch2 = grayimg(self.real_B[0][:, 1])
        self.real_B_0_ch3 = grayimg(self.real_B[0][:, 2])
        self.real_B_0_ch4 = grayimg(self.real_B[0][:, 3])
        self.real_A_1 = self.real_A[1]
        self.real_B_1_ch1 = grayimg(self.real_B[1][:, 0])
        self.real_B_1_ch2 = grayimg(self.real_B[1][:, 1])
        self.real_B_1_ch3 = grayimg(self.real_B[1][:, 2])
        self.real_B_1_ch4 = grayimg(self.real_B[1][:, 3])
        self.real_A_2 = self.real_A[2]
        self.real_B_2_ch1 = grayimg(self.real_B[2][:, 0])
        self.real_B_2_ch2 = grayimg(self.real_B[2][:, 1])
        self.real_B_2_ch3 = grayimg(self.real_B[2][:, 2])
        self.real_B_2_ch4 = grayimg(self.real_B[2][:, 3])
    ...
    def forward(self):
        ...
        self.fake_B_0_ch1 = grayimg(self.fake_B[0][:, 0])
        self.fake_B_0_ch2 = grayimg(self.fake_B[0][:, 1])
        self.fake_B_0_ch3 = grayimg(self.fake_B[0][:, 2])
        self.fake_B_0_ch4 = grayimg(self.fake_B[0][:, 3])
        self.fake_B_1_ch1 = grayimg(self.fake_B[1][:, 0])
        self.fake_B_1_ch2 = grayimg(self.fake_B[1][:, 1])
        self.fake_B_1_ch3 = grayimg(self.fake_B[1][:, 2])
        self.fake_B_1_ch4 = grayimg(self.fake_B[1][:, 3])
        self.fake_B_2_ch1 = grayimg(self.fake_B[2][:, 0])
        self.fake_B_2_ch2 = grayimg(self.fake_B[2][:, 1])
        self.fake_B_2_ch3 = grayimg(self.fake_B[2][:, 2])
        self.fake_B_2_ch4 = grayimg(self.fake_B[2][:, 3])
    ...
    def backward_D(self):
        ...
        self.loss_D_real = []
        for i in range(len(self.real_A)):
            # ch 0:t1, 1:t2, 2:flair, 3:t1c.
            # db0:Other, db1:CBICA, db2:TCIA
            # db0:miss t1c, db1:miss t2, db2:miss flair
            idx = [0, 1, 2, 3]
            if i == 0:
                del idx[3]
            elif i == 1:
            ...
            for j, id in enumerate(idx):
                real_A_1 = self.real_A[i][:, id]
                fake_B_1 = self.fake_B[i][:, id]
                real_B_1 = self.real_B[i][:, id]
                # fake_AB = torch.stack((real_A_1, fake_B_1), 1)
                # real_AB = torch.stack((real_A_1, real_B_1), 1)
    ...
    def backward_G(self):
        ...
        for i in range(self.D_size):
            ...
            for j, id in enumerate(idx):
                ...
                # Apply the element-wised weights for l1 loss.
                # l1loss = self.criterionL1(fake_B_1, real_B_1)
                # mask_1[mask_1<0] = 0
                # mask_1[mask_1>1] = 1
                # l1loss_elem = l1loss_elem * mask_1
                # l1loss = l1loss_elem.sum()/mask_1.sum()
                # self.loss_G_L1.append(l1loss * self.opt.lambda_L1)
    ...
    def save_imgs(self,real_batch, visuals, opt, batch_num, real_path, fake_path):
        gt = real_batch['Seg_0']
        for index in range(gt.shape[0]):
            for d_id in range(3):
                for ch_id in range(1,5):
                    real_img = visuals[f'real_B_{d_id}_ch{ch_id}'][index]
                    syn_img = visuals[f'fake_B_{d_id}_ch{ch_id}'][index]
                    filename = f"{batch_num}_{opt.batch_size}_{index}_d{d_id}_ch{ch_id}"
