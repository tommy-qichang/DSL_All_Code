...
class Pix2PixModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--delta_consistency', type=float, default=1.0, help='weight for consistency loss')
            parser.add_argument('--datatype', type=float, default=1.0, help='weight for perceptual loss')
        parser.add_argument('--subset', type=int, default=0, help='Assigned subset dataset id')
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['real_A', 'fake_B', 'real_B','key']
    ...
    def backward_G(self):
        ...
        if self.fake_B.shape[1] != 3:
            for i in range(self.fake_B.shape[1]):
                fake_B_i = self.fake_B[:,i]
                real_B_i = self.real_B[:,i]
                pred_feat = self.vgg_model(torch.stack([fake_B_i,fake_B_i,fake_B_i], dim=1))
                target_feat = self.vgg_model(torch.stack([real_B_i,real_B_i,real_B_i], dim=1))
        ...
        self.loss_G_perceptual_all = self.loss_G_perceptual[0]
        for i in range(len(self.loss_G_perceptual)-1):
            self.loss_G_perceptual_all += self.loss_G_perceptual[i+1]
        self.loss_G_perceptual = self.loss_G_perceptual_all
