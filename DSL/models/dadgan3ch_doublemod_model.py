...
class Dadgan3chDoublemodModel(BaseModel):
        ...
        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            ...
            for i in range(self.D_size * 2):
                self.netD.append(networks.define_D(opt.input_nc + opt.output_nc, opt.ndf, opt.netD,
                                                   ...
                                                   self.gpu_ids, opt.memory_gan, current_memory_gan_id=i % 2))
    ...
    def backward_D(self):
        ...
        for i in range(len(self.real_A)):
            ...
            for j, id in enumerate(idx):
                ...
                pred_fake = self.netD[i * 2 + j](fake_AB.detach())
                self.loss_D_fake.append(self.criterionGAN(pred_fake, False))
                pred_real = self.netD[i * 2 + j](real_AB)
    ...
    def backward_G(self):
        ...
        for i in range(self.D_size):
            ...
            for j, id in enumerate(idx):
                ...
                pred_fake = self.netD[i * 2 + j](fake_AB)
