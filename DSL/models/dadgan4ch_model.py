...
class Dadgan4chModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            self.netD = []
            # for i in range(10):
            for i in range(self.D_size * 4):
                self.netD.append(networks.define_D(2, opt.ndf, opt.netD,
                                                   opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain,
                                                   self.gpu_ids, opt.memory_gan, current_memory_gan_id=i % 4))
    ...
    def backward_D(self):
        ...
        for i in range(len(self.real_A)):
            for j in range(4):
                ...
                pred_fake = self.netD[i * 4 + j](fake_AB.detach())
                self.loss_D_fake.append(self.criterionGAN(pred_fake, False))
                pred_real = self.netD[i * 4 + j](real_AB)
    ...
    def backward_G(self):
        ...
        for i in range(self.D_size):
            for j in range(4):
                ...
                fake_AB = torch.stack((real_A_1, fake_B_1), 1)
                real_AB = torch.stack((real_A_1, real_B_1), 1)
                mask_1 = self.mask[i][:, j]

                pred_fake = self.netD[i * 4 + j](fake_AB)
                ...
                l1loss_elem = self.criterionL1(fake_B_1, real_B_1)
                # mask_1[mask_1<0] = 0
                # mask_1[mask_1>1] = 1
